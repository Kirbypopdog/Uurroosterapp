...
- **Waar het inlogtoken staat** (#159): niet rechtstreeks in de opslag, maar via `bewaarToken()`, `leesToken()` en `wisToken()` in `data.js`. Het token staat in `localStorage`, dus je blijft ingelogd zolang het geldig is: zeven dagen, overal, ook in een gewoon browsertabblad. Bewuste keuze van Victor. **Wat dat betekent**: de app bevat ziekmeldingen, dus gezondheidsgegevens (#152), en er is nergens een uitlog-na-inactiviteit. Op een computer die door meer dan één persoon gebruikt wordt blijft de vorige persoon ingelogd tot het token verloopt of tot iemand op uitloggen drukt. Wordt dat ooit een probleem, dan is de oplossing een uitlog-na-inactiviteit en niet het terugzetten naar `sessionStorage`: dat laatste maakt alleen het inloggen weer lastig zonder het gat te dichten. `leesToken()` kijkt in ALLEBEI de opslagen, want er kan nog een token uit `sessionStorage` staan van vóór deze wijziging en de terugval bij geblokkeerde `localStorage` schrijft daar ook naartoe; `wisToken()` leegt er allebei. De controle vóór de eerste paint in `index.html` doet hetzelfde nog eens inline, want `data.js` is daar nog niet geladen
