...
- **Waar het inlogtoken staat** (#159): niet rechtstreeks in `sessionStorage`, maar via `bewaarToken()`, `leesToken()` en `wisToken()` in `data.js`. Draait de app vanaf het beginscherm (`display-mode: standalone`, of `navigator.standalone` op iOS), dan gaat het token naar `localStorage` en blijf je ingelogd zolang het geldig is, zeven dagen. In een gewoon browsertabblad blijft het in `sessionStorage` en ben je het kwijt als je het tabblad sluit. Dat onderscheid is er omdat de app ziekmeldingen bevat, dus gezondheidsgegevens (#152), en er nergens een uitlog-na-inactiviteit is: op een gedeelde computer zou een blijvende aanmelding betekenen dat de volgende persoon ziet wie er ziek is. Een app op een beginscherm staat per definitie op iemands eigen toestel. `leesToken()` kijkt in ALLEBEI de opslagen, want die kan tussen twee keer openen verschillen; `wisToken()` leegt er allebei. De controle vóór de eerste paint in `index.html` doet hetzelfde nog eens inline, want `data.js` is daar nog niet geladen
