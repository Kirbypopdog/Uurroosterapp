...
function handleLogin(e) {
    ...
    if (user) {
        ...
        sessionStorage.setItem('hetvlot_user', JSON.stringify(user));
        showApp();
    } else {
...
function checkSession() {
    const savedUser = sessionStorage.getItem('hetvlot_user');
    if (savedUser) {
...
function renderShiftCard(shift) {
    ...
    return `<div class="${cardClass}" data-shift-id="${shift.id}">
        ...
        <div class="shift-card-footer">
            <span class="shift-team-badge team-${shift.team}">${DataStore.settings.teams[shift.team].name}</span>
            <button class="shift-delete-btn" data-shift-id="${shift.id}">×</button>
        </div>
    </div>`;
}
